...
export const writeChangeSetAtom = atom(
  null,
  (get, set, [updates, previousVersion]: [readonly Update[], number]) => {
    if (updates.length !== 0)
    ...
    const lastKnownVersion = getSyncedVersion(state);
    if (lastKnownVersion !== previousVersion) {
      console.error(
        `lastKnownVersion ${lastKnownVersion} !== previousVersion ${previousVersion}`,
      );
      return;
    }

    console.dir({
      xVer: lastKnownVersion,
      x: state,
